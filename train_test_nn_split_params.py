...
if __name__ == '__main__':
    ...
    parser.add_argument('--datasize', type=int, default=1440, help='NN hidden layer sizes per layer paramater')
    parser.add_argument('--alpha', type=float, default=10, help='NN alpha paramater')
    parser.add_argument('--solver', type=str, default='lbfgs', help='NN solver paramater')
    args = parser.parse_args()
    
    dataset_dir = 'dataset_%s_%s.dil' % (args.datatype, args.model)
    clf_save_dir = os.path.join('classifiers', 'nn_' + args.datatype + '_split%d_' + args.model + '.dil')
    ...
    for split in range(3):
        ...
        clf = neural_network.MLPClassifier(solver=args.solver, alpha=args.alpha, hidden_layer_sizes=(args.datasize, 2), random_state=1)
        t = time.time()
