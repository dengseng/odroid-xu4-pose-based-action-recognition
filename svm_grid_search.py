...
if __name__ == '__main__':
    ...
    params_dict = {"C": np.logspace(0,3,16), 
                   "gamma": np.logspace(-8, -6, 11)}
    svm = svm.SVC(kernel='rbf')
    
    start = time.time()
    # Fit the grid search
    search_coords_40 = GridSearchCV(estimator=svm, param_grid=params_dict, cv=3)
    ...
    start = time.time()
    # Fit the grid search
    search_dists_40 = GridSearchCV(estimator=svm, param_grid=params_dict, cv=3)
    ...
    start = time.time()
    # Fit the grid search
    search_coords = GridSearchCV(estimator=svm, param_grid=params_dict, cv=3)
    ...
    start = time.time()
    # Fit the grid search
    search_dists = GridSearchCV(estimator=svm, param_grid=params_dict, cv=3)
